...
- `src/ui/dev-loop/` contains the one-shot client view and pure manual action-option model. It initializes content through the content boundary, derives structurally validated options from current state, submits only enabled supported actions to the public ordered turn resolver, and renders all core-returned state changes as debug output.
...
- `src/game-core/simulation/` contains the pure doctrine-shift rule, the placeholder project-metric derivation, and the public ordered `resolveTurn` pipeline. The fixed phase tuple makes validation, player action, metrics, factions, contradictions, external relations, rivals, events, output validation, ending checks, and result assembly explicit. Doctrine shift selects an ideology axis by stable ID and applies a bounded increase or decrease; the following metric phase adjusts only player mobilization and legitimacy by one. Other action consequences and later state-system phases remain unsupported.
- `src/game-core/session/` contains the pure synchronous `runScriptedSession` orchestrator. It validates an initial state, resolves an ordered action list exclusively through `resolveTurn`, and returns the initial state, final state, and ordered turn results without importing content or runtime infrastructure.
- `src/content/minimal/` contains the five-entry Task 4.2 starting-archetype catalog, the unchanged provisional rival configuration, content-specific entry and catalog validation, default and archetype-specific core-validated `GameState` initializers, and the content-owned fixed ten-action Task 9.2 scripted-session helper. Content imports core contracts, validators, and public orchestration APIs; `game-core` does not import content.
...
All other folders in the planned layout remain deferred until their scoped tasks. The catalog remains in `content/minimal`; the broader catalog-oriented `content` subfolders shown above remain planned. The other planned `ui` folders also remain deferred; Task 9.1 adds only `ui/dev-loop`.
