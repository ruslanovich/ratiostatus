...
Task 4.2 adds catalog-focused coverage for a minimum of five entries; unique archetype and project IDs; per-entry content validation; complete bounded project and faction metric records; non-empty ideology, institution, and faction data; local archetype and faction reference integrity; lookup and unknown-ID rejection; and valid `GameState` initialization for every start. Simple variety checks require at least two ideology profiles, three project-metric profiles, and three represented institution categories. They intentionally do not assert balance or final gameplay outcomes. Default-state determinism and the existing exact ten-turn scripted proof remain regression-covered.
