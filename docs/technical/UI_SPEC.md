...
The trigger constructs a structured player `shift_doctrine` intent for the displayed first axis, with an `increase` direction and magnitude `10`, and passes the current state and action to `resolveTurn`. The UI stores the returned state and `TurnResult`; it does not calculate changes. The debug result panel renders the result identifier, accepted action kind, changed field, before/after values, and causal action identifier supplied by the core.
