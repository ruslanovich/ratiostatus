...
Clicking an enabled option passes its action to `resolveTurn`. A UI-level resolver gate returns without calling the core for disabled options, and native disabled buttons enforce the same interaction constraint. After one successful resolution, all triggers remain disabled. The UI stores the returned state and `TurnResult`; it does not calculate changes. The debug result panel renders the result identifier and accepted action kind, then lists every returned state change with its field, before/after values, and causal action identifier supplied by the core.
